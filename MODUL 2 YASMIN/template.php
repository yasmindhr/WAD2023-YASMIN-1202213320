...
// silakan taruh code kalian di bawah
if ($tinggi_badan == null or $berat_badan== null){
    $error=  "TIDAK BOLEH KOSONG";
}else{ 
    $tinggi = $tinggi_badan/100;
    $rumusbmi = ($berat_badan/($tinggi*$tinggi));
    if ($rumusbmi <= 18.4) {
        $status = "Underweight";
    } elseif ($rumusbmi >=18.5 && $rumusbmi<=24.9){
        $status = "Normal";
    }elseif ($rumusbmi >=25.0 && $rumusbmi <= 39.9){
        $status = "Overweight";
    }elseif ($rumusbmi >= 40.0){
        $status = "Obese";
    }
}
}
